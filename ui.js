...
const STORE = {
  weapons: [
    { id:"blaster",  name:"Бластер", icon:"./assets/blaster.svg", price:0,   desc:"Стартовое. Быстрая стрельба, средний урон.", type:"weapon" },
    { id:"smg",      name:"SMG",     icon:"./assets/smg.svg",     price:250, desc:"Очень быстро, меньше урон. Отлично на толпу.", type:"weapon" },
    { id:"shotgun",  name:"Дробовик",icon:"./assets/shotgun.svg", price:450, desc:"Плёт по площади. Близко — ваншоты.", type:"weapon" },
  ],
  gear: [
    { id:"armor",  name:"Броня",   icon:"./assets/armor.svg",  price:220, desc:"+40 брони в начале каждой игры.", type:"gear" },
    { id:"boots",  name:"Ботинки", icon:"./assets/boots.svg",  price:180, desc:"+10% скорость передвижения.",     type:"gear" },
  ],
  skins: [
    { id:"skin_camo", name:"Скин: Камо", icon:"./assets/skin_camo.svg", price:160, desc:"Меняет цвет персонажа на камуфляж.", type:"skin" },
    { id:"skin_neon", name:"Скин: Неон", icon:"./assets/skin_neon.svg", price:160, desc:"Неоновый стиль для ночных рейдов.", type:"skin" },
  ],
  boosts: [
    { id:"medkit", name:"Аптечка", icon:"./assets/medkit.svg", price:90, desc:"+35 HP (применяется на старте).", type:"boost" },
  ]
};

const STORAGE_KEY = "bks_shooter_state_v1";
...
async function cloudPull(){
  ...
  const values = await TG.cloudGet(["bks_state"]);
  if(values && values.bks_state){
    try{
      const cloud = JSON.parse(values.bks_state);
      if((cloud.coins||0) > (state.coins||0)) state = { ...defaultState(), ...cloud };
    }catch(e){}
  }
}
...
async function cloudPush(){
  if(!window.TG || !TG.isTelegram()) return;
  try{ await TG.cloudSet({ "bks_state": JSON.stringify(state) }); }catch(e){}
}
...
function updateHud(){
  ...
  const d = DISTRICTS.find(x=>x.id===state.districtId);
  $("hudDistrict").textContent = d ? d.name : "—";
}
  ...
  items.forEach(it=>{
    ...
    btn.addEventListener("click", ()=>{
      ...
      if(it.type==="weapon"){
        ...
        state.equipped.skin = it.id;
      } else if(it.type==="gear"){
        const idx = state.equipped.gear.indexOf(it.id);
        if(idx>=0) state.equipped.gear.splice(idx,1);
        else state.equipped.gear.push(it.id);
      } else if(it.type==="boost"){
        const idx = state.equipped.boosts.indexOf(it.id);
        if(idx>=0) state.equipped.boosts.splice(idx,1);
        else state.equipped.boosts.push(it.id);
      }
...
function resetAll(){
  if(!confirm("Сбросить прогресс (монеты/покупки/рекорды)?")) return;
  state = defaultState();
  save();
  setScreen("start");
  renderDistricts();
...
function telegramInfo(){
  if(window.TG){
    ...
    if(u){
      ...
      $("tgInfo").textContent = "Открыто в Telegram.";
    }
  } else {
